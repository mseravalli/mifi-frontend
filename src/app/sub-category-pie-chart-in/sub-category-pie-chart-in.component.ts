...
export class SubCategoryPieChartInComponent implements OnInit {
  ...
  ngOnChanges(changes: {[propKey: string]: SimpleChange}) {
    this.subcategories
      .filter(x => x.selected)
      .map(x => SubCategoryPieChartInComponent.colorTable[x.name] = x.color);
    SubCategoryPieChartInComponent.data = this.subCategoryPieChartIn;
    google.charts.setOnLoadCallback(this.drawChart);
  }

  private drawChart() {
    var data = google.visualization.arrayToDataTable( SubCategoryPieChartInComponent.data, false);
    var categories = [];
    ...
    var options = {
      ...
      colors: Utils.assignColors(categories, SubCategoryPieChartInComponent.colorTable)
    };
  
    var chart = new google.visualization.PieChart(document.getElementById('sub-category-pie-chart-in'));
