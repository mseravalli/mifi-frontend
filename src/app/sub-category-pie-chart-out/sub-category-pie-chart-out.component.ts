...
export class SubCategoryPieChartOutComponent implements OnInit {
  ...
  ngOnChanges(changes: {[propKey: string]: SimpleChange}) {
    this.subcategories
      .filter(x => x.selected)
      .map(x => SubCategoryPieChartOutComponent.colorTable[x.name] = x.color);
    SubCategoryPieChartOutComponent.data = this.subCategoryPieChartOut;
    google.charts.setOnLoadCallback(this.drawChart);
  }

  private drawChart() {
    var data = google.visualization.arrayToDataTable( SubCategoryPieChartOutComponent.data, false);
    var categories = [];
    ...
    var options = {
      ...
      colors: Utils.assignColors(categories, SubCategoryPieChartOutComponent.colorTable)
    };
  
    var chart = new google.visualization.PieChart(document.getElementById('sub-category-pie-chart-out'));
