...
export class CategoryPieChartOutComponent implements OnInit {
  ...
  private drawChart() {
    var data = google.visualization.arrayToDataTable(
      CategoryPieChartOutComponent.ccc,
      false
    );
    var categories = [];
    for (var i = 1; i < CategoryPieChartOutComponent.ccc.length; ++i) {
    ...
    var options = {
      ...
      colors: Utils.assignColors(
        categories,
        CategoryPieChartOutComponent.colorTable
      ),
    };

    var chart = new google.visualization.PieChart(
      document.getElementById("category-pie-chart-out")
    );
