...
export class RecurringComponent implements OnInit {
  ...
  ngOnChanges(changes: { [propKey: string]: SimpleChange }) {
    ...
    for (var i = 0; i < this.categories.length; ++i) {
      ...
      for (var j = 0; j < this.categories[i].subcategories.length; ++j) {
        ...
        RecurringComponent.colorTable[categoryName + "/" + subCategoryName] =
          subCategoryColor;
      }
    }
  ...
  private drawChart() {
    for (const recurring_period in RecurringComponent.raw_data) {
      for (const categorySubcategory in RecurringComponent.raw_data[
        recurring_period
      ]) {
        var data = google.visualization.arrayToDataTable(
          RecurringComponent.raw_data[recurring_period][categorySubcategory],
          false
        );
        var options = {
          colors: Utils.assignColors(
            ["total", "min", "max", categorySubcategory, categorySubcategory],
            RecurringComponent.colorTable
          ),
          isStacked: "true",
          seriesType: "bars",
          series: {
